...
let carModels = inventory.map((inv) => inv.car_model);
...
let carYears = inventory.map((inv) => inv.car_year);
...
function BMWA(entry) {
  return entry.car_make === "BMW" || entry.car_make === "Audi";
}

let BMWAndAudi = inventory.filter(BMWA);
